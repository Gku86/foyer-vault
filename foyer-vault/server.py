...
def _sync_vapid_pem(data: dict) -> None:
    pem = data.get("private_pem") or ""
    if not pem:
        return
    raw = pem.encode("utf-8") if isinstance(pem, str) else pem
    DATA.mkdir(parents=True, exist_ok=True)
    if PEM.exists() and PEM.read_bytes() == raw:
        return
    tmp = PEM.with_suffix(".tmp")
    tmp.write_bytes(raw)
    tmp.replace(PEM)


def vapid_key_arg(vapid: dict):
    """pywebpush only loads PEM via a file path (or a Vapid object). A PEM string hits from_string() and fails."""
    _sync_vapid_pem(vapid)
    if PEM.exists():
        return str(PEM)
    return vapid.get("private_pem") or ""
...
def send_push(title: str, body: str, url: str, exclude: str, member_id: str, username: str = "", member_ids=None) -> dict:
    ...
    if sent < 1 and last_error:
        code, message = classify_push_error(last_error, last_status)
        return {"ok": False, "sent": 0, "error": message, "code": code}
    if sent < 1 and matched == 0:
        return {"ok": True, "sent": 0}
    return {"ok": True, "sent": sent}


def classify_push_error(message: str, status) -> tuple:
    ...
    if status in (404, 410):
        return (
            "gone",
            "Cet appareil n’écoute plus. Ouvrez HomeBudget dessus (icône d’accueil) → Paramètres du compte → Activer, puis renvoyez.",
        )
    if status == 413:
        return "payload", "Le message est trop long pour une notification."
    if status == 429:
        return "rate", "Trop d’envois d’un coup. Réessayez dans une minute."
    if status == 403 or "forbidden" in text or "vapid" in text or "jwt" in text or "deserialize" in text:
        return (
            "auth",
            "L’abonnement n’est plus reconnu. Sur l’iPhone et l’ordinateur : Paramètres du compte → Activer, puis renvoyez.",
        )
    if any(
        token in text
        for token in (
        ...
        return (
            "network",
            "Le coffre n’a pas pu joindre le service de notifications. Vérifiez qu’Home Assistant a accès à Internet.",
        )
    return (
        "refused",
        "Le service de notifications a refusé l’envoi. Réactivez les notifications sur l’appareil, puis renvoyez.",
    )
...
def deliver_webpush(subscription_info: dict, payload: str, vapid: dict) -> None:
    ...
    parsed = urlparse(str(subscription_info.get("endpoint") or ""))
    aud = "%s://%s" % (parsed.scheme, parsed.netloc) if parsed.scheme and parsed.netloc else ""
    claims = {"sub": "https://foyer.fchvtn.ovh"}
    if aud:
        claims["aud"] = aud
    base = {
        "subscription_info": subscription_info,
        "data": payload,
        "vapid_private_key": vapid_key_arg(vapid),
        "vapid_claims": claims,
    ...
    try:
        webpush(headers={"Urgency": "high"}, **base)
        return
    except TypeError:
        pass
    webpush(**base)
